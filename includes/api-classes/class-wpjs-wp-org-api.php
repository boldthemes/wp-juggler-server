...
final class WPJSWpOrgApi {
	...
	private function json_get_request( $url, $headers = [], $options = [] ) {
		...
		if ( JSON_ERROR_NONE !== json_last_error() ) {
			return new WP_Error( 'WP.org API Error', 'Failed to decode JSON: ' . json_last_error_msg() );
		}
	...
	private function get_request( $url, $headers = [], $options = [] ) {
		...
		if ($response_code != '200' && $response_code != '201') {
			...
			return new WP_Error('WP.org API Error', 'Wrong response code from WP.org');

...
class WPJS_Mailer
{
	...
	public static function wpjs_send_api_alert_start( $site_id )
	{
		$will_send = get_post_meta($site_id, 'wp_juggler_api_down', true);

		if(!$will_send){
			update_post_meta($site_id, 'wp_juggler_api_down', true);
			$serialized_datetime = serialize( new DateTime(current_time('mysql'), new DateTimeZone(wp_timezone_string())) );
			update_post_meta($site_id, 'wp_juggler_api_down_time', $serialized_datetime );
		} else {
			return;
		}
		...
		$subject_template = "API on {{ site_name }} is down";
		$subject = WPJS_Mailer::parse_template( $subject_template, $args );
		$message_template = "API on {{ site_name }} ( url: {{ site_url }} ) stopped responding at {{ site_time }}";
		$message = WPJS_Mailer::parse_template( $message_template, $args );
	...
	public static function wpjs_send_api_alert_end( $site_id )
	{
		if (! metadata_exists('post', $site_id, 'wp_juggler_api_down')){
			return;
		}
		...
		if($will_send){
			$serialized_datetime = get_post_meta($site_id, 'wp_juggler_api_down_time', true);
			if ($serialized_datetime) {
				$prev_time = unserialize($serialized_datetime);
			}
			...
			if ($prev_time instanceof DateTime) {
				...
				update_post_meta($site_id, 'wp_juggler_api_down', false);
				$serialized_datetime = serialize( new DateTime(current_time('mysql'), new DateTimeZone(wp_timezone_string())) );
				update_post_meta($site_id, 'wp_juggler_api_down_time', $serialized_datetime );
			} else {
				return;
		...
		$subject_template = "API on {{ site_name }} is up";
		$subject = WPJS_Mailer::parse_template( $subject_template, $args );
		$message_template = "API on {{ site_name }} ( url: {{ site_url }} ) started responding at {{ site_time }}. The API was down for {{ interval }}";
		$message = WPJS_Mailer::parse_template( $message_template, $args );

      ...
      <v-row class="mb-4">
        <v-col cols="12" md="3">
          <v-card>
            ...
            <v-card-text class="text-medium-emphasis">
              <div class="d-flex py-3 justify-space-between pt-0">
                <div>
                  ...
                  <v-row align="center" no-gutters>
                    <v-col class="text-h2" cols="12" v-if="props.item.wp_juggler_health_data"> {{
                      props.item.wp_juggler_health_data.critical }} </v-col>
                ...
                <div>
                  ...
                  <v-row align="center" no-gutters>
                    <v-col class="text-h2" cols="12" v-if="props.item.wp_juggler_health_data"> {{
                      props.item.wp_juggler_health_data.recommended }} </v-col>
                    <v-col class="text-h2" cols="12" v-else> ? </v-col>
                  </v-row>
                </div>
              </div>
              ...
              <div class="d-flex py-3 justify-space-between pt-0">
                <div v-if="props.item.wp_juggler_wordpress_version">WordPress version: {{
            ...
            <v-btn text="Full Report" append-icon="mdi-chevron-right" class="mb-5 ml-5 mt-4 text-none text-caption"
              @click="openHealth(props.item)"></v-btn>
          </v-card>
        </v-col>
        ...
        <v-col cols="12" md="3">
          <v-card>
            ...
            <v-card-text class="text-medium-emphasis pt-0 mb-4">
                ...
                <div>
                  ...
                  <v-row align="center" no-gutters>
                    <v-col class="text-h2" cols="12"> {{
                      item.wp_juggler_uptime_stats.summary[selectedUptimePeriod].front }}
                    </v-col>
                  </v-row>
                </div>
                <div>
                  ...
                  <v-row align="center" no-gutters>
                    <v-col class="text-h2" cols="12"> {{ item.wp_juggler_uptime_stats.summary[selectedUptimePeriod].api
                      }}
                    </v-col>
                  </v-row>
                </div>
              ...
              <div>Total failed checks: {{ parseInt(item.wp_juggler_uptime_stats.summary[selectedUptimePeriod].front) +
                parseInt(item.wp_juggler_uptime_stats.summary[selectedUptimePeriod].api) }}</div>
            </v-card-text>

            <v-divider></v-divider>

            <v-btn text="Full Report" append-icon="mdi-chevron-right" class="mb-5 ml-5 mt-4 text-none text-caption"
              @click="openUptime(props.item)"></v-btn>
          </v-card>
        </v-col>
        ...
        <v-col cols="12" md="3">
          <v-card>
                ...
                <div>
                  ...
                  <v-row align="center" no-gutters>
                    <v-col class="text-h2" cols="12" v-if="props.item?.wp_juggler_plugins_summary"> {{
                      props.item.wp_juggler_plugins_summary.updates_num }} </v-col>
                    <v-col class="text-h2" cols="12" v-else> ? </v-col>
                  </v-row>
                </div>
              </div>
              <div class="d-flex py-3 justify-space-between pt-0">
                <div v-if="props.item?.wp_juggler_plugins_summary">Recorded vulnerabilities: {{
                  props.item.wp_juggler_plugins_summary.vulnerabilities_num }}</div>
            ...
            <v-btn text="Manage Themes & Plugins" append-icon="mdi-chevron-right"
              class="mb-5 ml-5 mt-4 text-none text-caption" @click="openThemesPlugins(props.item)"
              ref="themesButton"></v-btn>
          </v-card>
        </v-col>
        ...
        <v-col cols="12" md="3">
          <v-card>
            ...
            <v-btn text="All notices" append-icon="mdi-chevron-right"
              class="mb-5 ml-5 mt-4 text-none text-caption" @click="openNotices(props.item)"
              ref="noticesButton"></v-btn>
          </v-card>
        </v-col>
      </v-row>
    </td>
